...
def integer_class_loss(
    ...
    Like multiclass-classification, predicitons are a probabilites for each possible integer,
    but the ICL aims to penalise close predictions less than far-off ones:
    For a target of 3 and a close prediction of softmax([1,3,10,5,5,3,1]) and a far-off prediciton of softmax([10,3,1,5,5,3,1]),
    the categorical cross-entropy produces the same loss for both predicitons (5.0154) depsite the close prediction having a higher probability near the target.

    ICL instead computes the absolute error, or squared error, between each of the possible integers and the true target.
    These errors are then normalised, weighted by the predicted probabilites, and summed.
    ...
    Arguments:
        int_probs: (*,integers) tensor of predicted probabilites
        ...
        reduction: 'mean' return the avergae ICL, 'sum' sum the ICLs, 'none', return the individual ICLs
    """
