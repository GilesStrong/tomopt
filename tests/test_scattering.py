...
def make_comparison(geant_df:pd.DataFrame, 
    ...
    for name, sim in (("pdg", pdg_scattering), ("param_geant", pgeant_scattering)):
        ...
        tests[var][name]['bulk95_mean_fdiff'] = np.abs(sim[var][sim[var] <= cut95].mean()-geant_df.loc[geant_df[var]<=cut95,var].mean())/geant_df.loc[geant_df[var]<=cut95,var].mean()
        tests[var][name]['bulk98_std_fdiff'] = np.abs(sim[var][sim[var] <= cut98].std()-geant_df.loc[geant_df[var]<=cut98,var].std())/geant_df.loc[geant_df[var]<=cut98,var].std()
...
def test_scatter_models_Al_1cm_1GeV_normal():
    with h5py.File(PKG_DIR / "data/geant_scatter_validation.hdf5", "r") as geant_data:
        ...
        assert check_scatter_tests(results["tests"], pdg_pmin=0.15, pgeant_pmin=None, test="bulk95_mean_fdiff" , ignore_vars=["dx_vol","dy_vol","dtheta_x_vol","dtheta_y_vol"])  
...
def test_scatter_models_Fe_1cm_1GeV_normal():
    with h5py.File(PKG_DIR / "data/geant_scatter_validation.hdf5", "r") as geant_data:
        ...
        assert check_scatter_tests(results["tests"], pdg_pmin=0.1, pgeant_pmin=None, test="bulk95_mean_fdiff" , ignore_vars=["dx_vol","dy_vol","dtheta_x_vol","dtheta_y_vol"])  
