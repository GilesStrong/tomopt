...
def test_volume_wrapper_scan_volumes(state, mocker):  # noqa F811
    ...
    mocker.spy(vw.opts["mu_opt"], "zero_grad")
    mocker.spy(vw.opts["z_pos_opt"], "zero_grad")
    mocker.spy(vw.opts["sig_opt"], "zero_grad")
    mocker.spy(vw.opts["norm_opt"], "zero_grad")
    mocker.spy(vw.opts["norm_opt"], "step")
    mocker.spy(vw.opts["mu_opt"], "step")
    mocker.spy(vw.opts["z_pos_opt"], "step")
    mocker.spy(vw.opts["sig_opt"], "step")
    ...
    if state == "train":
        ...
        assert vw.opts["mu_opt"].zero_grad.call_count == 2
        assert vw.opts["z_pos_opt"].zero_grad.call_count == 2
        assert vw.opts["sig_opt"].zero_grad.call_count == 2
        assert vw.opts["norm_opt"].zero_grad.call_count == 2
        assert vw.opts["norm_opt"].step.call_count == 2
        assert vw.opts["mu_opt"].step.call_count == 2
        assert vw.opts["z_pos_opt"].zero_grad.call_count == 2
        assert vw.opts["sig_opt"].step.call_count == 2
    else:
        assert cb.on_backwards_begin.call_count == 0
        assert cb.on_backwards_end.call_count == 0
        assert vw.opts["mu_opt"].zero_grad.call_count == 0
        assert vw.opts["z_pos_opt"].zero_grad.call_count == 0
        assert vw.opts["sig_opt"].zero_grad.call_count == 0
        assert vw.opts["norm_opt"].zero_grad.call_count == 0
        assert vw.opts["norm_opt"].step.call_count == 0
        assert vw.opts["mu_opt"].step.call_count == 0
        assert vw.opts["z_pos_opt"].step.call_count == 0
        assert vw.opts["sig_opt"].step.call_count == 0
...
def test_volume_wrapper_fit_epoch(mocker):  # noqa F811
    ...
    vw.fit_params = FitParams(n_mu_per_volume=1000, mu_bs=1000, cbs=[cb], trn_passives=trn_py, val_passives=val_py, passive_bs=1)
...
def test_volume_wrapper_fit(mocker):  # noqa F811
    ...
    vw.fit(n_epochs=2, n_mu_per_volume=1000, mu_bs=1000, passive_bs=1, trn_passives=trn_py, val_passives=val_py, cbs=[cb])
...
def test_volume_wrapper_predict(mocker):  # noqa F811
    ...
    vw = HeatMapVolumeWrapper(
        volume,
        mu_opt=partial(optim.SGD, lr=2e4, momentum=0.95),
        z_pos_opt=partial(optim.Adam, lr=2e0),
        sig_opt=partial(optim.Adam, lr=2e2),
        norm_opt=partial(optim.Adam, lr=2e2),
        loss_func=VoxelX0Loss(target_budget=1, cost_coef=0.15),
    )
    ...
    preds = vw.predict(py, n_mu_per_volume=1000, mu_bs=1000, pred_cb=pred_cb, cbs=cbs)
